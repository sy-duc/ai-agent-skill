...
| Category | Tình huống / Keyword trong đề                                                                                | Service / Giải pháp chính                                                   | Ghi chú                                                                                                                                                                                                                                              |
| -------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **🔒 Security** | | | |
| | API Gateway access control limited IP                                                                        | **Create a resource policy**                                                | Resource Policy là một cách không mất tiền để định nghĩa quyền kiểm soát truy cập API Gateway theo IP                                                                                                                                                |
| | Active Directory + AWS Organizations                                                                         | **IAM Identity Center**                                                     | Liên kết người dùng Active Directory để cho phép truy cập account trong Organization                                                                                                                                                                 |
| | DB credentials be encrypted and rotated                                                                      | **Secrets Manager**                                                         | AWS Secrets Manager là service giúp lưu trữ các thông tin quan trọng như db credentials, username, password, v.v.. một cách an toàn với cơ chế làm mới (rotation) định kỳ                                                                            |
| | Temporary access to S3 & tích hợp tốt với IAM Identity Center                                                | **IAM Roles Anywhere**                                                      | IAM Roles Anywhere cho phép workload on-premises sử dụng credentials tạm thời thông qua certificate-based authentication                                                                                                                             |
| | Encryption at rest                                                                                           | **AWS KMS**                                                                 | Với yêu cầu mã hóa đầu cuối (encryption at rest) thì solution là KMS                                                                                                                                                                                 |
| | Encryption in transit                                                                                        | **AWS Certificate Manager (ACM) SSL/TLS certificates**                      | Với yêu cầu mã hoá trên đường truyền (encryption in transit) thì cần sử dụng giao thức an toàn có mã hoá như HTTPS TLS SSL                                                                                                                           |
| | Cerfiticate in CloudFront to upload the photo files to the S3                                                | **us-east-1**                                                               | Certificate cho CloudFront bắt buộc phải tạo ở region us-east-1                                                                                                                                                                                      |
| | Use Amazon CloudFront                                                                                        | **CloudFront origin access control (OAC)**                                  | OAC là cách thức được AWS khuyến nghị để giới hạn việc truy cập đến S3 bắt buộc phải thông qua CloudFront, từ đó giúp tăng tính security, tránh việc người dùng truy cập trực tiếp đến S3 mà bỏ qua CloudFront                                       |
| | Malicious attacks                                                                                            | **Amazon GuardDuty**                                                        | GuardDuty giúp phát hiện hoạt động bất thường, đáng ngờ (malicious)                                                                                                                                                                                  |
| | Security findings                                                                                            | **AWS Security Hub**                                                        | Security Hub giúp tổng hợp và quản lý các phát hiện security (security findings)                                                                                                                                                                     |
| | DDoS attack on the website                                                                                   | **AWS WAF by rate-based rules**                                             | WAF có thể hạn chế tấn công DDoS thông qua Rate-based rule                                                                                                                                                                                           |
...
| | Automatic key rotation                                                                                       | **AWS Managed KMS keys (SSE-KMS)**                                          | Chỉ KMS mới có automatic rotation, tiết kiệm hơn so với customer managed keys (vì không mất phí lưu trữ & quản lý key)                                                                                                                               |
...
| | Review permissions + over-privileged                                                                         | **IAM Access Analyzer**                                                     | IAM Access Analyzer được thiết kế chuyên biệt để phân tích permissions và phát hiện unused permissions (quyền được cấp mà không dùng) hoặc over-privileged access (quyền cấp thừa)                                                                   |
| | External users + MOST securely + AWS Organizations                                                           | **IAM Identity Center**                                                     | IAM Identity Center quản lý tập trung quyền truy cập vào AWS Account / Console cho các user                                                                                                                                                          |
| | OS access to private instances                                                                               | **Session Manager**                                                         | Session Manager là chức năng thuộc service Systems Manager cho phép truy cập thẳng vào OS mà không cần SSH keys hay thông qua bastion host                                                                                                           |
| | Restrict application to specific prefix                                                                      | **Access Point Policies**                                                   | S3 Access Points cho phép tạo các điểm truy cập (access point) vào S3 với để kiểm soát truy cập theo prefix tương ứng                                                                                                                                |
| | Third-party certificate authority CA                                                                         | **Import certificate into AWS Certificate Manager (ACM)**                   | ACM không thể tạo certificate được ký bởi third-party CA                                                                                                                                                                                             |
| | Automatic security controls/guardrails                                                                       | **AWS Control Tower**                                                       | AWS Control Tower giúp setup và bảo vệ multi-account AWS environment (AWS Organizations) một cách tự động                                                                                                                                            |
| | SQL injection + XSS                                                                                          | **AWS WAF**                                                                 | AWS WAF được thiết kế chuyên biệt để bảo vệ web applications khỏi các cuộc tấn công layer 7 như SQL injection và XSS                                                                                                                                 |
| | DDoS attacks                                                                                                 | **AWS Shield**                                                              | AWS Shield bảo vệ chống DDoS attacks (layer 3/4)                                                                                                                                                                                                     |
| | Automate OS updates/patching                                                                                 | **AWS Systems Manager Patch Manager**                                       | Systems Manager Patch Manager được thiết kế chuyên biệt để automate OS patching và inventory management cho cả Windows và Linux                                                                                                                      |
| | Vulnerability assessment + monthly reports                                                                   | **Amazon Inspector**                                                        | Amazon Inspector chuyên về vulnerability assessment và security findings cho EC2 instances                                                                                                                                                           |
| | Multiple AWS accounts + centralized control                                                                  | **AWS Organizations + Service Control Policies (SCP)**                      | AWS Organizations với Service Control Policies (SCPs) được thiết kế để manage permissions across multiple accounts                                                                                                                                   |
| | Serverless auth/authz                                                                                        | **Amazon Cognito + Lambda@Edge**                                            | Amazon Cognito là serverless authentication service, perfect cho scalable user management, Lambda@Edge chạy tại CloudFront edge locations → low latency authorization                                                                                |
| | No static credentials + secure authentication                                                                | **IAM database authentication**                                             | IAM database authentication loại bỏ hoàn toàn static credentials                                                                                                                                                                                     |
| | Discover personally identifiable information (PII) + S3 buckets                                              | **Amazon Macie**                                                            | Amazon Macie được thiết kế chuyên biệt cho PII discovery và data classification trong S3                                                                                                                                                             |
| | Control Tower + Prevent/Block deployment                                                                     | **Proactive Controls**                                                      | Proactive controls là loại guardrails giúp ngăn chặn trước khi resource được tạo, hoạt động bằng cách validate CloudFormation templates và block deployment nếu vi phạm policy được quy định sẵn                                                     |
| | Quản lý và xác thực đăng nhập người dùng (authentication)                                                    | **Cognito User Pool**                                                       |                                                                                                                                                                                                                                                      |
| | Cấp quyền cho người dùng thao tác với AWS Service (authorization)                                            | **Cognito Identity Pool**                                                   |                                                                                                                                                                                                                                                      |
| **🌐 Networking** | | | |
| | Distribution rights content around the world                                                                 | **Route53 Geolocation Routing Policy**                                      | Cho phép định tuyến traffic dựa trên vị trí địa lý của user                                                                                                                                                                                          |
| | Traffic (to S3, DynamoDB) from the application must not travel across the internet                           | **Gateway Endpoint**                                                        | Gateway Endpoint cho S3 là dịch vụ miễn phí, cho phép kết nối đến S3 với traffic đi trực tiếp trong đường truyền nội bộ của AWS, không qua internet                                                                                                  |
| | Improve the performance of the static website                                                                | **CloudFront**                                                              | CloudFront là CDN service, cho phép cache các loại data như content tĩnh ở điểm cung cấp dịch vụ gần với người dùng nhất (edge locations), từ đó giúp giảm độ trễ                                                                                    |
...
| | Slow for International Customers                                                                             | **AWS Global Accelerator**                                                  | Global Accelerator là giải pháp giúp tăng tốc, cải thiện hiệu suất cho phép các kết nối trên khắp thế giới đến hệ thống một cách nhanh nhất thông qua mạng lưới nội bộ toàn cầu của AWS                                                              |
| | Traffic to Amazon S3 does not use the public endpoint                                                        | **Gateway VPC endpoint**                                                    | VPC endpoint cho phép application trong VPC access đến các service khác một cách an toàn, không đi qua internet                                                                                                                                      |
| | Connect on-premise to VPC securely                                                                           | **Site-to-Site VPN**                                                        | Để kết nối on-premise đến VPC an toàn, có mã hoá trên đường truyền thì có thể sử dụng Site-to-Site VPN                                                                                                                                               |
| | Traffic TCP                                                                                                  | **Network Load Balancer, Global Accelerator**                               | Network Load Balancer là lựa chọn tối ưu vì được thiết kế đặc biệt cho Layer 4 (TCP/UDP)                                                                                                                                                             |
| | Dynamic content globally                                                                                     | **CloudFront**                                                              | Dù CloudFront chủ yếu dùng để cache static content tại edge locations gần user, giúp giảm độ trễ nhưng nó cũng giúp cho việc delivery dyanmic content đến người dùng một cách nhanh hơn                                                              |
| | Connect on-premise to AWS accounts with DirectConnect                                                        | **AWS Transit Gateway**                                                     | Transit Gateway cho phép quản lí kết nối tập trung giữa nhiều accounts và môi trường on-premises thông qua một cổng kết nối duy nhất                                                                                                                 |
| | Routing users to the closest server                                                                          | **Route53 Geolocation Routing Policy**                                      | Route53 Geolocation Routing Policy giúp routing người dùng đến server gần nhất                                                                                                                                                                       |
| | Single-digit latency                                                                                         | **Direct Connect**                                                          | Độ trễ hàng mili giây (single-digit latency) kết nối đến môi trường on-premise thì nghĩ đến Direct Connect                                                                                                                                           |
| | Third-party SaaS + private connectivity                                                                      | **AWS PrivateLink**                                                         | AWS PrivateLink là dịch vụ cho phép tạo private endpoint trong VPC của mình để truy cập các AWS services hoặc third-party services nằm trên AWS mà không cần đi qua internet                                                                         |
| | Networking inspection                                                                                        | **AWS Gateway Load Balancer**                                               | Gateway Load Balancer là service chuyên dụng cho việc kiểm tra traffic (traffic inspection)                                                                                                                                                          |
| | Container with the LEAST operational overhead                                                                | **App2Container**                                                           | App2Container là service chuyên dụng để container hoá các ứng dụng một cách tự động, giảm effort thủ công                                                                                                                                            |
|         |                                                                                                              | **ECS Fargate**                                                             | ECS Fargate là serverless platform để chạy container mà không cần quản lý hạ tầng bên dưới                                                                                                                                                           |
| | Auto Scaling Group cần scale tự động                                                                         | **target tracking scaling**                                                 | Target tracking scaling là cơ chế tự động scale EC2 dựa trên ngưỡng metric do mình định nghĩa (Vd: CPU 40%). Khi traffic tăng đột biến, sẽ tự động scale out ngay lập tức và scale in khi traffic giảm                                               |
| | TCP/UDP traffic                                                                                              | **Network Load Balancer & AWS Global Accelerator**                          | Global Accelerator hỗ trợ TCP/UDP raw traffic (còn CloudFront là CDN cho HTTP/HTTPS)                                                                                                                                                                 |
| | Transport layer                                                                                              | **Network Load Balancer**                                                   | Cần Network Load Balancer để handle TCP/UDP traffic, cung cấp high availability và distribute traffic across instances                                                                                                                               |
| | Centrally managed networking                                                                                 | **AWS RAM**                                                                 | AWS RAM cho phép quản lý tập trung và chia sẻ các networking components như subnet, vpc cho các account khác, giảm thiểu operational overhead vì không cần quản lý riêng lẻ                                                                          |
| | Expose service privately to other accounts                                                                   | **PrivateLink with NLB/ALB**                                                | AWS PrivateLink là dịch vụ cho phép kết nối private giữa các VPC, AWS services. Traffic đi hoàn toàn trong mạng nội bộ AWS, đảm bảo bảo mật cao và độ trễ thấp                                                                                       |
| | Query strings/parameters routing                                                                             | **Application Load Balancer (ALB)**                                         | Application Load Balancer (ALB) là lựa chọn duy nhất hỗ trợ query parameter-based routing (định tuyến dựa trên query string)                                                                                                                         |
| **💻 Compute** | | | |
| | Workflow                                                                                                     | **Step Functions**                                                          | Step Functions là serverless service giúp điều phối & tự động hoá, cho phép build các luồng công việc (workflow) phức tạp                                                                                                                            |
| | Image processing jobs                                                                                        | **Lambda, ECS Fargate, AWS Batch**                                          | Chạy job mà tốn ít effort, không phải quản lý toàn bộ cơ sở hạ tầng. Nếu jobs chạy trên 15 phút sẽ loại ngay Lambda, ưu tiên chọn các solution managed, serverless như ECS Fargate, Batch. Nếu thời gian dưới 15 phút thì thường sẽ lựa chọn Lambda. |
| | EC2 instances run 100% CPU                                                                                   | **Auto Scaling Group**                                                      | Tự động scale khi bị bottleneck (nút thắt cổ chai) ở EC2 Processing Tier                                                                                                                                                                             |
| | SQS queue fills up                                                                                           | **ApproximateNumberOfMessages**                                             | Trigger scale EC2 khi số lượng message đang có trong queue nhiều mà không xử lý hết                                                                                                                                                                  |
| | Application stateless need to scale                                                                          | **Auto Scaling Group & Spot Instances**                                     | Spot Fleet giúp tiết kiệm, dù có thể bị thu hồi nhưng application stateless và có khả năng chịu lỗi cao nên không ảnh hưởng                                                                                                                          |
| | Prevent sharing same hardware                                                                                | **Spread Placement Group**                                                  | Spread Placement Group được thiết kế chính xác cho mục đích đặt vị trí EC2 đảm bảo mỗi instance chạy trên phần cứng vật lý riêng biệt (separate underlying hardware)                                                                                 |
| | Hybrid environment + on-premises + container                                                                 | **ECS Anywhere**                                                            | ECS Anywhere phù hợp cho chạy container on-premise vì là service cho phép quản lý và chạy containers trên hạ tầng bên ngoài AWS nhưng vẫn sử dụng AWS ECS control plane để điều khiển từ AWS Console                                                 |
| | SAP application + high memory utilization                                                                    | **Memory optimized instances**                                              | Memory optimized instances (R-series, X-series) được thiết kế specifically cho workloads như SAP và in-memory databases                                                                                                                              |
| | High volume write operations                                                                                 | **Provisioned IOPS SSD storage**                                            | Provisioned IOPS SSD storage được thiết kế đặc biệt cho high-performance, write-heavy workloads                                                                                                                                                      |
| | High performance computing (HPC) workload + low latency + high throughput                                    | **Cluster placement groups**                                                | Cluster placement groups được thiết kế chuyên biệt cho HPC workloads                                                                                                                                                                                 |
| | Giảm độ trễ cho Lambda                                                                                       | **Provisioned Concurrency**                                                 | Provisioned concurrency là tính năng giữ cho môi trường thực thi Lambda luôn sẵn sàng, có thể chạy luôn mà không cần khởi tạo lại từ đầu, giúp loại bỏ cold start, đảm bảo latency thấp và ổn định                                                   |
| | Kiểm soát khả năng scale của Lambda                                                                          | **Reserved Concurrency**                                                    | Reserved concurrency giới hạn số function chạy đồng thời                                                                                                                                                                                             |
| **🚚 Migration** | | | |
| | Migrate lift-and-shift or migrate server                                                                     | **Application Migration Service**                                           | Thực hiện migrate server từ on-premise lên AWS thông qua hình thức lift-and-shift (di chuyển nguyên trạng)                                                                                                                                           |
| | Migrate hundreds of TB or PB of data to AWS                                                                  | **AWS Snowball**                                                            | Với lượng data hàng trăm TB hay hàng PB thì việc migrate hầu như sẽ thực hiện qua Snowball Edge                                                                                                                                                      |
| | Collect data/usage/configuration on-premises servers                                                         | **AWS Application Discovery Service**                                       | AWS Application Discovery Service là service cho phép tự động thu thập thông tin về servers on-premises (CPU, memory, disk, network, dependencies) thông qua Agent cài ở phía on-premise                                                             |
| **🗄️ Storage** | | | |
| | Deploy the static content                                                                                    | **S3 + CloudFront**                                                         | S3 + CloudFront là giải pháp tối ưu cho static content, scale không giới hạn                                                                                                                                                                         |
| | Storage solution for application on premises                                                                 | **S3 File Gateway**                                                         | S3 File Gateway cung cấp NFS/SMB interface để ứng dụng on-premises có thể truy cập S3 như là local file system và có local cache để đảm bảo việc truy cập với độ trễ thấp (low-latency) cho data truy cập thường xuyên (frequently accessed data)    |
| | Solution storage for on-premises block storage                                                               | **Volume Gateway**                                                          | Volume Gateway tương thích với block storage (iSCSI protocol)                                                                                                                                                                                        |
| | Storage solution tương thích NFS khi migrate                                                                 | **Elastic File System (Amazon EFS)**                                        | EFS là fully managed NFS service, hỗ trợ NFS                                                                                                                                                                                                         |
| | Fileshare for Linux instance                                                                                 | **Elastic File System (Amazon EFS)**                                        | EFS là managed service cho phép các EC2 instances Linux truy cập cùng lúc vào cùng một file system                                                                                                                                                   |
| | Sử dụng S3 không biết trước tần suất                                                                         | **S3 Intelligent-Tiering**                                                  | S3 Intelligent-Tiering tự động chuyển đổi giữa các storage class dựa trên tần suất truy cập thực tế, tính khả dụng cao, chỉ trả phí theo usage thực tế                                                                                               |
| | Data transfer between regions                                                                                | **S3 Cross-Region Replication (CRR)**                                       | S3 Cross-Region Replication (CRR) giúp truyền data giữa các region                                                                                                                                                                                   |
| | Share data between multiple instances                                                                        | **Amazon Elastic File System (EFS)**                                        | EFS là file system được thiết kế chuyên để share data giữa nhiều EC2 instances đồng thời                                                                                                                                                             |
| | Multi-protocol + NFS and SMB                                                                                 | **FSx for NetApp ONTAP**                                                    | FSx for NetApp ONTAP là dịch vụ duy nhất trong các lựa chọn có thể hỗ trợ đồng thời cả NFS và SMB protocols trên cùng một file system                                                                                                                |
| | Accessed once or twice per year + immediately available                                                      | **Glacier Instant Retrieval**                                               | Glacier Instant Retrieval phù hợp cho yêu cầu truy cập ít trong năm đầu (năm 1, 2 lần) với độ trễ thấp nhất (immediately available) và chi phí tiết kiệm hơn Standard-IA                                                                             |
| | Millions of small files                                                                                      | **Group the files into large objects**                                      | Group các file nhỏ thành file to giúp giảm chi phí request cho S3 (hàng triệu files nhỏ = hàng triệu request)                                                                                                                                        |
| | Millions of objects                                                                                          | **S3 Batch Operations**                                                     | S3 Batch Operations cho phép thao tác hàng loạt trên hàng triệu/tỷ S3 objects                                                                                                                                                                        |
| | Accidentally deleted + Prevent data loss                                                                     | **Recycle Bin**                                                             | Recycle Bin là tính năng AWS cho phép khôi phục EBS Snapshot hoặc AMI đã bị xóa (dù nhầm lẫn) trong khoảng thời gian nhất định                                                                                                                       |
| | SMB clients + LEAST administrative overhead                                                                  | **FSx for Windows File Server**                                             | FSx for Windows File Server là managed service về file share storage cho Windows, hỗ trợ giao thức SMB protocol                                                                                                                                      |
| | Store petabytes data                                                                                         | **AWS S3**                                                                  | S3 là dịch vụ object storage có khả năng lưu trữ không giới hạn                                                                                                                                                                                      |
| | Identify S3 configuration across regions / S3 analytics / Monitor S3 compliance                              | **S3 Storage Lens**                                                         | S3 Storage Lens là công cụ analytics and monitoring chính thức của AWS để theo dõi cấu hình S3                                                                                                                                                       |
| | SMB file share                                                                                               | **S3 File Gateway (NFS/SMB interface)**                                     | S3 File Gateway cung cấp SMB interface để devices có thể write data trực tiếp                                                                                                                                                                        |
| | Shared storage + multiple EC2 instances                                                                      | **Amazon EFS**                                                              | Amazon EFS là shared file system có thể mount đồng thời trên multiple EC2 instances                                                                                                                                                                  |
| | SMB file share                                                                                               | **S3 File Gateway (NFS/SMB interface)**                                     | S3 File Gateway cung cấp SMB interface để devices có thể write data trực tiếp                                                                                                                                                                        |
| | SFTP + managed solution                                                                                      | **AWS Transfer Family**                                                     | AWS Transfer Family được thiết kế chuyên biệt cho managed SFTP services                                                                                                                                                                              |
| | Large data size + DynamoDB                                                                                   | **S3 + DynamoDB reference pattern**                                         | DynamoDB có giới hạn 400 KB/item. Với dữ liệu 700 KB và tiếp tục tăng, cần lưu trữ bên ngoài và S3 là giải pháp tối ưu cho lưu file giá rẻ, DynamoDB sẽ chỉ lưu các thông tin phụ của data (metadata) và đường dẫn đến data trên S3                  |
| | Media streaming + tiered by demand / content age                                                             | **S3 Standard**                                                             | Dùng cho content nhu cầu cao (phim mới): truy cập ngay lập tức, không mất phí retrieval |
|  |                                                                                                               | **S3 Standard-IA**                                                          | Dùng cho content nhu cầu thấp (phim cũ): chi phí lưu trữ thấp hơn, standard retrieval vẫn immediate access — đáp ứng yêu cầu "within 5 minutes" |
| **🗃️ Database** | | | |
| | Rapidly evolve schema                                                                                        | **DynamoDB**                                                                | DynamoDB là NoSQL Databse nên sẽ hỗ trợ schema linh hoạt                                                                                                                                                                                             |
| | Robust, continuously available, and resilient DynamoDB architecture to maintain a seamless gaming experience | **DynamoDB global tables**                                                  | Giúp đảm bảo tính khả dụng cao và khả năng chịu lỗi đồng thời có thể phục vụ được lượng người dùng trên khắp thế giới tốt hơn                                                                                                                        |
| | Data đọc nhiều, ghi ít                                                                                       | **ElastiCache**                                                             | DB dạng quan hệ mà data đọc nhiều, ghi ít thì có thể sử dụng ElastiCache để cache data                                                                                                                                                               |
| | Structured database must be scalable                                                                         | **Aurora Serverless**                                                       | Aurora Serverless hỗ trợ DB dạng quan hệ có khả năng tự scale để đáp ứng traffic                                                                                                                                                                     |
| | Backup tự động lưu trữ > 35 ngày                                                                             | **AWS Backup**                                                              | AWS Backup là managed service hỗ trợ backup tự động với thời gian lưu trữ vô thời hạn (RDS automated backups chỉ hỗ trợ lưu trữ tối đa 35 ngày)                                                                                                      |
| | Phục hồi db về thời điểm cụ thể                                                                              | **Point-in-time recovery**                                                  | Là cơ chế backup tăng tiến tự động của RDS, giúp có thể quay lại bất cứ thời điểm nào trong quá khứ (tối đa 35 ngày)                                                                                                                                 |
| | Database failovers                                                                                           | **Amazon RDS Proxy**                                                        | RDS Proxy giúp giảm thời gian failover cho RDS hoặc Aurora                                                                                                                                                                                           |
| | Session state + high availability                                                                            | **ElastiCache Redis**                                                       | Cache lưu độc lập bên trong ElastiCache, tách biệt khỏi web server, do đó nếu web server gặp sự cố thì cũng không ảnh hưởng. Có cơ chế replication data → đảm bảo không mất dữ liệu                                                                  |
| | IT infrastructure map + relationships                                                                        | **Graph database (Neptune)**                                                | Neptune là graph database được thiết kế chuyên biệt cho việc lập bản đồ relationships                                                                                                                                                                |
| | Third-party features + privileged access                                                                     | **RDS Custom**                                                              | RDS Custom for Oracle cho phép privileged access và customization cần thiết cho third-party features                                                                                                                                                 |
| | Many-to-many relationships + mutual connections                                                              | **Graph database (Neptune)**                                                | Amazon Neptune là graph database được thiết kế chuyên biệt cho relationship data. Many-to-many relationships được handle naturally trong graph databases                                                                                             |
| | RDS to Aurora migration                                                                                      | **Aurora read replica**                                                     | Từ RDS PostgreSQL có thể tạo trực tiếp Aurora read replica, data sẽ được đồng bộ giữa 2 instance này. Quá trình diễn ra không ảnh hưởng đến instance gốc, downtime cực ngắn                                                                          |
| **💬 Messaging** | | | |
| | Data must not be lost because of a scaling event                                                             | **Amazon Simple Queue Service (SQS)**                                       | SQS queue đóng vai trò buffer cho việc truyền data đến mỗi nhóm EC2, từ đó giúp đảm bảo không mất data khi EC2 scale                                                                                                                                 |
| | No data is lost & exactly-once processing                                                                    | **Amazon SQS FIFO**                                                         | SQS đảm bảo data không bị mất, SQS FIFO đảm bảo được việc chỉ xử lý message đúng 1 lần duy nhất (exactly-once processing)                                                                                                                            |
| | pub/sub pattern                                                                                              | **Amazon SNS**                                                              | SNS là service phù hợp đáp ứng cho pub/sub pattern - một publisher có thể gửi message đến nhiều subscribers                                                                                                                                          |
| | Email notification                                                                                           | **Amazon Simple Notification Service (SNS)**                                | SNS có thể gửi email notification trực tiếp hoặc integrate với Amazon Simple Email Service (SES)                                                                                                                                                     |
| **📊 Analytics / AI** | | | |
| | ETL (extract-transform-load)                                                                                 | **AWS Glue**                                                                | AWS Glue là dịch vụ ETL serverless, phù hợp cho việc extract-transform-load với ít effort nhất                                                                                                                                                       |
| | Sentiment analysis                                                                                           | **Amazon Comprehend**                                                       | Amazon Comprehend là dịch vụ chuyên dụng cho sentiment analysis để thực hiện phân tích thái độ                                                                                                                                                       |
| | Query data trên S3 không thường xuyên                                                                        | **Amazon Athena**                                                           | Athena là dịch vụ serverless cho phép query data trực tiếp trên S3. Vì là serverless nên sẽ thích hợp cho việc truy vấn occasional (không thường xuyên)                                                                                              |
| | Real-time                                                                                                    | **Kinesis Data Streams**                                                    | Kinesis Data Streams dành cho việc streaming data và processing data trong thời gian thực (real-time)                                                                                                                                                |
| | Search, real-time update                                                                                     | **OpenSearch**                                                              | OpenSearch Service chuyên cho việc search data, support search data real-time                                                                                                                                                                        |
| | Visualization                                                                                                | **QuickSight**                                                              | QuickSight là AWS native service cho việc hiển thị hoá data, support liên kết với OpenSearch                                                                                                                                                         |
| | Performance analysis                                                                                         | **CloudWatch Metrics**                                                      | CloudWatch metrics cung cấp sẵn các thông số performance quan trọng như CPU, memory, disk I/O, network để có thể thực hiện phân tích ngay lập tức. Trong khi CloudWatch logs không chứa performance metrics                                          |
| | Granularity 1-2 minutes                                                                                      | **CloudWatch Detailed Monitoring**                                          | Detailed monitoring là tính năng cho phép CloudWatch thu thập metrics với quãng 1 phút / lần                                                                                                                                                         |
| | Cost Analytics                                                                                               | **Cost and Usage Report → S3 → Athena → QuickSight**                        | QuickSight là dịch vụ BI chuyên dụng cho việc tạo dashboard và visualization từ dữ liệu cost, Athena là công cụ phù hợp nhất để query Cost and Usage Report trực tiếp từ S3                                                                          |
| | .csv files                                                                                                   | **Glue crawler + Athena**                                                   | Glue crawler tự động discover schema của .csv files trong S3                                                                                                                                                                                         |
| | .csv files                                                                                                   | **Glue crawler + Athena**                                                   | Glue crawler tự động discover schema của .csv files trong S3                                                                                                                                                                                         |
| | Real-time data ingestion                                                                                     | **Kinesis Data Streams**                                                    | Kinesis Data Streams là giải pháp tối ưu cho việc streaming data trong thời gian thực (real-time) từ nhiều nguồn khác nhau                                                                                                                           |
| | Detect unwanted content in images                                                                            | **Amazon Rekognition**                                                      | Amazon Rekognition là dịch vụ managed AI, tích hợp model có sẵn của AWS, dùng để phân tích ảnh, phát hiện nội dung không phù hợp như bạo lực, nội dung nhạy cảm... mà KHÔNG cần training model từ đầu                                                |
| **💰 Cost Optimization** | | | |
| | Cost breakdown by application                                                                                | **Cost allocation tags + Cost Explorer**                                    | Cost allocation tags cho phép quản lý và phân chia chi phí theo tag, và Cost Explorer cung cấp dashboard trực quan với khả năng tạo report, cập nhật thường xuyên tự động - tất cả đều hoàn toàn miễn phí                                            |
| | Long-running workloads                                                                                       | **Reserved Instances/Savings Plans**                                        | Cam kết sử dụng dài hạn (3 năm) + thanh toán trả trước toàn bộ (All Upfront) sẽ mang lại mức chiết khấu cao nhất, tối ưu chi phí tốt nhất                                                                                                            |
| | Cost optimizations across EC2/Auto Scaling group/EBS                                                         | **Configure AWS Compute Optimizer**                                         | AWS Compute Optimizer là dịch vụ được thiết kế chuyên biệt để phân tích và đưa ra các khuyến cáo về việc tối ưu chi phí cho EC2, Auto Scaling groups và EBS volumes                                                                                  |
| | Analyze cost and recommend optimizations                                                                     | **AWS Compute Optimizer**                                                   | AWS Compute Optimizer được thiết kế chuyên biệt để analyze và recommend optimizations cho AWS resources bao gồm EBS volumes                                                                                                                          |
| | Analyze cost and recommend optimizations                                                                     | **AWS Compute Optimizer**                                                   | AWS Compute Optimizer được thiết kế chuyên biệt để analyze và recommend optimizations cho AWS resources bao gồm EBS volumes                                                                                                                          |
| | Tracking chi phí theo từng phòng ban                                                                         | **Cost allocation tags**                                                    | AWS Cost allocation tags giúp gắn tag owner cho từng resource, cho phép tracking chi phí theo từng phòng ban                                                                                                                                         |
| **⚙️ Management / Governance** | | | |
| | Centrally manage + Self-service                                                                              | **AWS Service Catalog**                                                     | AWS Service Catalog là dịch vụ cho phép các tổ chức tạo và quản lý catalog tập trung các sản phẩm IT đã được phê duyệt (như CloudFormation templates, AMIs, applications) mà end users có thể tự triển khai thông qua giao diện web đơn giản         |
